...
# ---------------------------------------------------------------------------
@app.post("/api/tick")
def tick(x_owl_secret: str | None = Header(default=None)) -> dict[str, Any]:
    """Run a full watchlist scan + dispatch alerts. Externally triggered.
    ...
    try:
        ...
        return {
            ...
            "window_hours": hours,
            "stations_scanned": _STATE["last_tick_stations"],
            "flagged": flagged,
            "alerts_sent": alerts_sent,
            "duration_s": _STATE["last_tick_duration_s"],
        }
        ...
        _log.exception("/api/tick failed")
        return JSONResponse(
            status_code=500,
            content={
                "ok": False,
