...
# =============================================================================
# O.W.L. OBSERVATION CONTROL CENTER — NOC theme overlay.
# Injected as a SECOND <style> block so it layers on top of the base
# typography/layout CSS without risking cascade regressions.  Gives the
# app the deep-obsidian / HUD-bracket / mission-control aesthetic.
# =============================================================================
st.markdown("""<style>
/* Technical display fonts for the HUD.  Rajdhani for labels, Share Tech
   Mono for readouts, JetBrains Mono already loaded above. */
@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;500;600;700&family=Share+Tech+Mono&family=Barlow+Condensed:wght@500;600;700&display=swap');

/* ---------------------------------------------------------------------------
   Color tokens (scoped to app root so toggling is straightforward later)
   --------------------------------------------------------------------------- */
:root {
    --noc-bg-deep:        #050816;
    --noc-bg-panel:       #0b1220;
    --noc-bg-panel-alt:   #0f1a30;
    --noc-bg-elevated:    #111c36;
    --noc-border:         #1a2845;
    --noc-border-strong:  #2d3e5f;
    --noc-text-primary:   #e2e8f0;
    --noc-text-muted:     #94a3b8;
    --noc-text-dim:       #64748b;
    --noc-accent-cyan:    #00e5ff;
    --noc-accent-cyan-dim:#0891b2;
    --noc-accent-amber:   #ffb800;
    --noc-status-ok:      #00ff88;
    --noc-status-warn:    #ffbe2e;
    --noc-status-crit:    #ff3860;
    --noc-status-info:    #38bdf8;
    --noc-glow-cyan:      0 0 10px rgba(0, 229, 255, 0.50);
    --noc-glow-ok:        0 0 10px rgba(0, 255, 136, 0.55);
    --noc-glow-warn:      0 0 10px rgba(255, 190, 46, 0.55);
    --noc-glow-crit:      0 0 10px rgba(255, 56, 96, 0.55);
}

/* ---------------------------------------------------------------------------
   Deep obsidian background + subtle grid backdrop + scanline accent.
   The grid is drawn with two stacked linear-gradients so no image assets
   are needed.  `::after` adds a single moving scanline for ambience —
   opacity stays low so it never distracts from the data.
   --------------------------------------------------------------------------- */
section.main,
section.main > div.block-container,
[data-testid="stAppViewContainer"] {
    background-color: var(--noc-bg-deep) !important;
...
section.main > div.block-container {
    background-image:
        ...
        radial-gradient(ellipse at top, rgba(14, 165, 233, 0.05) 0%, transparent 55%);
...
[data-testid="stAppViewContainer"]::after {
    ...
    animation: noc-scan 12s linear infinite;
    z-index: 9999;
}
@keyframes noc-scan {
...
/* ---------------------------------------------------------------------------
   Primary text/headings — slightly brighter, tighter, tracked.
   --------------------------------------------------------------------------- */
section.main p,
section.main li,
...
section.main h4 {
    color: var(--noc-text-primary) !important;
    font-family: 'Rajdhani', 'Space Grotesk', 'Inter', sans-serif !important;
...
/* ---------------------------------------------------------------------------
   Core panel frame — corner brackets via pseudo-elements.  Any element
   with class `.noc-panel` gets the HUD treatment.  Corners can also be
   applied with `.noc-corner-all` on a wrapper div.
   --------------------------------------------------------------------------- */
.noc-panel {
    position: relative;
    background: linear-gradient(180deg, var(--noc-bg-panel) 0%, var(--noc-bg-panel-alt) 100%);
...
.noc-panel::before {                       /* top-left + top-right corners */
    top: -1px; left: -1px;
    border-width: 2px 0 0 2px;
}
.noc-panel::after {                        /* bottom-left + bottom-right */
    bottom: -1px; right: -1px;
    border-width: 0 2px 2px 0;
}
.noc-panel .noc-corner-tr,
.noc-panel .noc-corner-bl {
    position: absolute;
...
/* ---------------------------------------------------------------------------
   HUD readout (monospaced, glowing, tabular numbers).
   --------------------------------------------------------------------------- */
.noc-readout {
    font-family: 'Share Tech Mono', 'JetBrains Mono', ui-monospace, monospace !important;
    color: var(--noc-accent-cyan);
    letter-spacing: 0.08em;
...
.noc-light {
    ...
    border-radius: 50%;
    margin: 0 6px 0 0;
    animation: noc-blink 2.2s ease-in-out infinite;
}
.noc-light-ok   { background: var(--noc-status-ok);   box-shadow: var(--noc-glow-ok);   }
.noc-light-warn { background: var(--noc-status-warn); box-shadow: var(--noc-glow-warn); }
.noc-light-crit { background: var(--noc-status-crit); box-shadow: var(--noc-glow-crit); }
@keyframes noc-blink {
    0%, 100% { opacity: 1; }
    50%      { opacity: 0.45; }
}
...
/* ---------------------------------------------------------------------------
   Override the legacy ops banner to full mission-control bar.
   --------------------------------------------------------------------------- */
.ops-banner {
    background: linear-gradient(90deg,
        rgba(11, 18, 32, 0.95) 0%,
        rgba(15, 26, 48, 0.95) 50%,
        rgba(11, 18, 32, 0.95) 100%) !important;
    border: 1px solid var(--noc-border) !important;
    border-left: 2px solid var(--noc-accent-cyan) !important;
    border-radius: 0 !important;
...
/* ---------------------------------------------------------------------------
   Title block — keep the brand name, lift the color tone.
   --------------------------------------------------------------------------- */
.ops-title {
...
/* ---------------------------------------------------------------------------
   KPI / metric cards — HUD look with glowing accent borders.
   --------------------------------------------------------------------------- */
div[data-testid="stMetric"] {
    background: linear-gradient(180deg, var(--noc-bg-panel) 0%, var(--noc-bg-panel-alt) 100%) !important;
    border: 1px solid var(--noc-border) !important;
    border-left: 2px solid var(--noc-accent-cyan) !important;
    border-radius: 0 !important;
...
div[data-testid="stMetric"] [data-testid="stMetricValue"] {
    ...
    color: var(--noc-text-primary) !important;
    font-variant-numeric: tabular-nums;
    text-shadow: 0 0 12px rgba(226, 232, 240, 0.10);
}

/* ---------------------------------------------------------------------------
   Health card overrides — make the big percentage glow like a HUD readout.
   --------------------------------------------------------------------------- */
.health-card {
    background: linear-gradient(180deg, var(--noc-bg-panel) 0%, var(--noc-bg-panel-alt) 100%) !important;
    border: 1px solid var(--noc-border) !important;
...
/* ---------------------------------------------------------------------------
   Tabs — HUD underline with cyan accent + dark backdrop.
   --------------------------------------------------------------------------- */
.stTabs [data-baseweb="tab-list"] {
...
.stTabs [data-baseweb="tab"][aria-selected="true"] {
    color: var(--noc-accent-cyan) !important;
    border-bottom-color: var(--noc-accent-cyan) !important;
...
/* ---------------------------------------------------------------------------
   Tables — dark theme with cyan header accents.
   --------------------------------------------------------------------------- */
[data-testid="stDataFrame"] {
    background: var(--noc-bg-panel) !important;
    border: 1px solid var(--noc-border) !important;
    border-radius: 0 !important;
}
[data-testid="stDataFrame"] [role="columnheader"],
[data-testid="stDataFrame"] th {
    background: var(--noc-bg-elevated) !important;
    color: var(--noc-accent-cyan) !important;
    font-family: 'Rajdhani', sans-serif !important;
    text-transform: uppercase;
    letter-spacing: 0.12em !important;
    font-size: 0.74rem !important;
    border-bottom: 1px solid var(--noc-border-strong) !important;
}

/* ---------------------------------------------------------------------------
   Buttons — HUD style with cut corners (clip-path).
   --------------------------------------------------------------------------- */
.stButton > button,
[data-testid="stDownloadButton"] > button {
    background: linear-gradient(180deg, var(--noc-bg-elevated) 0%, var(--noc-bg-panel-alt) 100%) !important;
    color: var(--noc-accent-cyan) !important;
    border: 1px solid var(--noc-accent-cyan-dim) !important;
    border-radius: 0 !important;
    font-family: 'Rajdhani', sans-serif !important;
    text-transform: uppercase;
...
[data-testid="stDownloadButton"] > button:hover {
    ...
    box-shadow: inset 0 0 20px rgba(0, 229, 255, 0.15),
                0 0 20px rgba(0, 229, 255, 0.25) !important;
    text-shadow: 0 0 8px rgba(0, 229, 255, 0.55);
}
.stButton > button[kind="primary"] {
    background: linear-gradient(180deg, rgba(0, 229, 255, 0.20) 0%, rgba(8, 145, 178, 0.35) 100%) !important;
    color: var(--noc-text-primary) !important;
    border: 1px solid var(--noc-accent-cyan) !important;
    text-transform: uppercase;
    box-shadow: 0 0 0 1px rgba(0, 229, 255, 0.2),
                0 0 20px rgba(0, 229, 255, 0.30),
                inset 0 0 20px rgba(0, 229, 255, 0.10);
    text-shadow: 0 0 8px rgba(0, 229, 255, 0.6);
}

/* ---------------------------------------------------------------------------
   Inputs, selects — consistent dark HUD treatment.
   --------------------------------------------------------------------------- */
.stTextInput input,
.stNumberInput input,
.stDateInput input,
...
.stSelectbox [data-baseweb="select"]:focus-within > div {
    border-color: var(--noc-accent-cyan) !important;
    box-shadow: 0 0 0 1px var(--noc-accent-cyan),
                0 0 12px rgba(0, 229, 255, 0.25) !important;
...
/* ---------------------------------------------------------------------------
   Expanders — dark panels with cyan header accent.
   --------------------------------------------------------------------------- */
[data-testid="stExpander"] {
    background: var(--noc-bg-panel) !important;
    border: 1px solid var(--noc-border) !important;
    border-left: 2px solid var(--noc-accent-cyan-dim) !important;
    border-radius: 0 !important;
    margin-bottom: 10px;
}
...
/* ---------------------------------------------------------------------------
   Sidebar — slightly less deep than main, matches the NOC palette.
   --------------------------------------------------------------------------- */
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, var(--noc-bg-panel) 0%, var(--noc-bg-deep) 100%) !important;
...
/* ---------------------------------------------------------------------------
   Federal footer — matches the NOC theme.
   --------------------------------------------------------------------------- */
.fed-footer {
    background: var(--noc-bg-panel) !important;
    border-top: 2px solid var(--noc-accent-cyan) !important;
    color: var(--noc-text-primary) !important;
    box-shadow: 0 -10px 30px rgba(0, 229, 255, 0.08);
}
.fed-footer-cite, .fed-footer-cite strong {
    color: var(--noc-text-muted) !important;
}
.fed-footer-meta {
...
/* Horizontal rules as subtle HUD dividers */
hr {
    border: none !important;
    border-top: 1px solid var(--noc-border) !important;
    position: relative;
...
/* ---------------------------------------------------------------------------
   Captions, hints — slightly brighter on dark background.
   --------------------------------------------------------------------------- */
[data-testid="stCaptionContainer"] {
    color: var(--noc-text-dim) !important;
    font-size: 0.78rem !important;
...
/* ---------------------------------------------------------------------------
   "Worst 5 Right Now" strip cards — HUD emphasis.
   --------------------------------------------------------------------------- */
.worst-card {
    background: linear-gradient(180deg, var(--noc-bg-panel) 0%, var(--noc-bg-panel-alt) 100%) !important;
    border: 1px solid var(--noc-border) !important;
...
/* Print overrides still remove the scanline */
@media print {
...
_uptime_str = f"{_health_clean + _health_problem:03d}/{_health_total:03d}"
_banner_html = (
    '<div class="ops-banner">'
    '<div class="ops-banner-left">'
    # Pulsing "SCAN ACTIVE" light + mission identifier.  Left edge of
    # the mission-control bar — reads as "we are live and watching".
    f'<span class="noc-light noc-light-ok"></span>'
    '<span class="banner-meta">NETWORK SCAN ACTIVE</span>'
    '<span class="noc-divider">|</span>'
    '<span class="banner-meta">'
    f'NODES <span class="noc-readout">{_uptime_str}</span>'
    '</span>'
    '<span class="noc-divider">|</span>'
    '<span class="banner-meta">MISSION <span class="noc-readout">OWL-01</span></span>'
    '</div>'
    '<div class="ops-banner-right">'
    f'<span class="status-dot" style="background:{_status_color};'
    f'box-shadow:0 0 10px {_status_color};"></span>'
    f'<span class="status-label" style="color:{_status_color};">{_status_label}</span>'
    '<span class="noc-divider">|</span>'
    f'<span class="banner-meta">UTC</span>'
